...
class DepthFirst{
  ...
  Update(){    
    ...
    if(this.openSet.length <= 0){
      print("Unsolvable maze DeapthFirst");
      return;
    }
    ...
    if(!this.completedPathCell){
      for(i = 0; i < this.openSet.length; i++){
        ...
        if(this.openSet[i].pos.x == this.endPos.x && this.openSet[i].pos.y == this.endPos.y){
          ...
          print("Solved DepthFirst");
          return;
        }
      }
    }
  
    //Expands the most recently added cell
    this.MakeNewCells(this.openSet[this.openSet.length - 1]);
  }
  ...
  MakeNewCells(expandingCell){
    ...
    //Removes the cell to be expanded from the expandable cell list
    this.openSet.splice(this.openSet.length - 1,1);
